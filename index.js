...
function lotsOfArt(array) {
  return array.filter(a => a.paintings >100).map(a => a.name);
}
...
function artistInfo(array,artist){
  return array.filter(a => a.name === artist)[0].bio;
}
...
function artistByCountry(array,artistHome){
  return array.filter(a => a.nationality=== artistHome).map(a => a.name);
}
